...
function indekseGoreFenomen(fenomen,indeks) {
  ...
  return `${indeks}. indekste bulunan fenomen: ${fenomen[indeks].profile}`;
}
console.log (indekseGoreFenomen(fenomenler,8));
...
function fenomenSil(fenomenDizi , indeks) {
  let yeniDizi = [...fenomenDizi];

  yeniDizi.splice(indeks,1);

  return yeniDizi;
}
console.log (fenomenSil(fenomenler,8));
...
function fenomenEkle(fenomenDizisi,sira,profil,takipci,gönderi,uygulama) {
  const yeniFeno = {
  "number": sira,
  "profile": profil,
  "followers": takipci,
...
for (let i = 0 ; i< fenomenlerDizisi.lenght;i++) 
{


if (fenomenlerDizini[i].followers > 100000000) {
  enFenomen.push (fenomenlerDizini[i].profile);
 }
}
return enFenomen;
}
...
function fenomenGonderimSayisi(fenomenDizisi , profilAdi) {
...
function platformaGoreCokGonderiYapanFenomen(fenomenlerDizini,platformAdi){
  let enCokGonderiFenomen = fenomenlerDizini
  .filter((fenomen) => fenomen.platform === platformAdi && fenomen.posts !== "NA")
  .reduce((enCokGonderi, fenomen) => {
    if (fenomen.posts > enCokGonderi.posts) {
      return fenomen;
    } else {
      return enCokGonderi;
    }
...
return enCokGonderiFenomen.profile;
}
